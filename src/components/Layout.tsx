...
import { Menu, X, ChevronDown, ChevronRight, CheckCircle2, BookOpen, Terminal, Lock } from 'lucide-react';
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const { isCompleted, getModuleProgress, canAccessModule, canAccessTopic } = useProgress(modules);
  const auth = useAuth();
      ...
      <div className="p-5 border-b border-border">
        <Link to="/" className="flex items-center gap-3" onClick={() => setSidebarOpen(false)}>
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Do zero ao sênior (+ extras)</p>
          </div>
        </Link>
      </div>
              ...
              >
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium text-foreground block truncate flex items-center gap-2">
                    {!moduleUnlocked && <Lock className="h-4 w-4 text-muted-foreground flex-shrink-0" />}
                    {mod.id}. {mod.title}
                  </span>
                  {progress > 0 && (
        ...
        <header className="lg:hidden sticky top-0 z-30 bg-card/90 backdrop-blur border-b border-border px-4 py-3 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            {auth.user ? (
              <>
                {auth.role === "admin" && (
                  <Link to="/admin" className="text-xs px-2 py-1 rounded-md border border-border hover:bg-muted/40">
                    Admin
                  </Link>
                )}
                <button
                  className="text-xs px-2 py-1 rounded-md border border-border hover:bg-muted/40"
              ...
              <Link to="/login" className="text-xs px-2 py-1 rounded-md border border-border hover:bg-muted/40">
                Entrar
              </Link>
            )}
          </div>
