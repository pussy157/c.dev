...
export const useProgress = (modules: Module[]) => {
  ...
  const saveProjectImage = useCallback((topicId: string, dataUrl: string) => {
    const current = getStoredProgress();
    saveProgress({
      ...current,
  ...
  const getProjectImage = useCallback((topicId: string) => {
    return progress.projectImages?.[topicId] || null;
  }, [progress]);
  ...
  const canAccessModule = useCallback((moduleId: number) => {
    ...
    return true;
  }, [progress, moduleTopicIds]);

  const canAccessTopic = useCallback((moduleId: number, topicId: string) => {
    ...
    return true;
  }, [progress, moduleTopicIds, canAccessModule]);
