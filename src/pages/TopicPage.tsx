...
import { ArrowLeft, ArrowRight, FileQuestion, Info, Lock, CheckCircle2, Image as ImageIcon } from 'lucide-react';
    ...
    return (
      <Layout>
        ...
        <div className="bg-card border border-border rounded-xl p-6 text-center">
          ...
          {prevTopic && (
            <Button onClick={() => navigate(`/topico/${prevTopic.moduleId}/${prevTopic.id}`)} className="gap-2">
  ...
  return (
    <Layout>
      ...
      <div className="space-y-5">
        {Array.isArray(content) && content.length > 0 ? content.map((block, i) => {
          switch (block.type) {
            case 'text':
      ...
      {/* Ações de conclusão (lesson/quiz/project) */}
      <div className="mt-10 space-y-4">
        {topic.type === 'lesson' && (
          <div className="p-6 bg-card rounded-xl border border-border">
            <h3 className="text-lg font-bold text-foreground mb-2">Concluir tópico</h3>
            <p className="text-sm text-muted-foreground mb-4">
              Quando você terminar os mini-treinos acima, marque este tópico como concluído para liberar o próximo.
            </p>
            {completed ? (
